...
def test_plot_fog_freq(fic, tpcw, mps, v):
    ...
    fic.return_value.value_counts.return_value.compute.side_effect = AttributeError
    v.plot_fog_frequency()
    assert fic.return_value.value_counts.return_value.sort_index.return_value.cumsum.return_value.sort_index.return_value.plot.call_count == 4
    assert vc.sort_index.return_value.cumsum.return_value.sort_index.return_value.plot.call_count == 4
